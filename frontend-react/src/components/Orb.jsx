...
const Orb = ({ state = 'idle', analyser = null }) => {
  ...
  useEffect(() => {
    if (orbInstanceRef.current) {
      orbInstanceRef.current.setState(state);
    }
  }, [state]);

  useEffect(() => {
    if (orbInstanceRef.current) {
      orbInstanceRef.current.setAnalyser(analyser);
    }
  }, [analyser]);
